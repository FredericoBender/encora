...
func initialModel() model {
	...
	m := model{
		inputs: make([]textinput.Model, args),
	}
...
func ReadInputs() map[string]string {
	...
	modelInputs := initialModel()
	if err := tea.NewProgram(modelInputs).Start(); err != nil {
		fmt.Printf("could not start program: %s\n", err)
		os.Exit(1)
	}
	for _, x := range modelInputs.inputs {
		allInputs[x.Placeholder] = x.Value()
	}
	return allInputs
}
